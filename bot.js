...
const notifyModmailLink = async (discordUserId) => {
  ...
  let userChannel = channels.find(channel => channel.type == 0 && channel.parentId && channel.parentId == MODMAIL_CATEGORY && channel.topic && channel.topic.search(discordUserId) != -1);  const poeAccount = await getPoeTftStateLinkByDiscordId(discordUserId);
  const infoEmbed = {
